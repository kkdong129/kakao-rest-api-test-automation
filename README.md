...
- Step 3. 사용자 정보 조회: 전송 완료 후 내 계정 정보(닉네임 등)를 조회하여 최종 세션 확인
### 2. Test_02: Negative Flow (예외 케이스)
API의 견고함을 검증하기 위한 부정 테스트 케이스입니다.
- TC-01 (401): 유효하지 않은 토큰(Invalid Token) 사용 시 차단 여부
- TC-02 (404): 잘못된 URL 엔드포인트 접근 시 대응 확인
- TC-03 (400): 필수 파라미터(template_object) 누락 시 에러 핸들링
- TC-04 (400): JSON 문법 오류(Syntax Error)가 포함된 데이터 전송 시 차단
